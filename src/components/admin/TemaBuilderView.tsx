      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className={`lg:col-span-7 space-y-4 ${visualizacaoMobile === 'preview' ? 'hidden lg:block' : 'block'}`}>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 p-1 bg-slate-900 border border-slate-800 rounded-xl">
            ...
            <button
              type="button"
              onClick={() => setSecaoEditor('checkout')}
              className={`py-2 px-1.5 rounded-lg text-xs font-bold flex flex-col sm:flex-row items-center justify-center gap-1.5 transition ${
          ...
          {/* 5. SEÇÃO CHECKOUT TRANSPARENTE & PAGAMENTOS */}
          {secaoEditor === 'checkout' && (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-5 animate-in fade-in">
              <div className="border-b border-slate-800 pb-3 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-black text-white flex items-center gap-2">
                    ...
                    Checkout Transparente & Métodos de Pagamento
                  </h3>
                  <p className="text-xs text-slate-400">
                    Defina quais formas de pagamento ficam disponíveis nesta campanha e personalize as mensagens.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  {/* Pix */}
                  <div
                    onClick={() => handleUpdateCheckout(prev => ({
                      ...prev,
                      metodos: { ...prev.metodos, pix: !prev.metodos.pix }
                    }))}
                    className={`p-4 rounded-xl border cursor-pointer transition select-none flex flex-col justify-between gap-3 ${
                      checkoutSeguro.metodos.pix
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        checkoutSeguro.metodos.pix ? 'bg-emerald-500 text-slate-950' : 'bg-slate-800 text-slate-500'
                      }`}>
                        {checkoutSeguro.metodos.pix ? '✓' : ''}
                      </span>
                    </div>
                    <div>
                      <h4 className="text-xs font-black text-white">Pix Instantâneo</h4>
                      <p className="text-[10px] text-slate-400 mt-0.5">QR Code + Copia e Cola com baixa automática</p>
                    </div>
                  </div>
                  ...
                  <div
                    onClick={() => handleUpdateCheckout(prev => ({
                      ...prev,
                      metodos: { ...prev.metodos, cartao: !prev.metodos.cartao }
                    }))}
                    className={`p-4 rounded-xl border cursor-pointer transition select-none flex flex-col justify-between gap-3 ${
                      checkoutSeguro.metodos.cartao
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        checkoutSeguro.metodos.cartao ? 'bg-blue-500 text-slate-950' : 'bg-slate-800 text-slate-500'
                      }`}>
                        {checkoutSeguro.metodos.cartao ? '✓' : ''}
                      </span>
                    </div>
                    <div>
                      <h4 className="text-xs font-black text-white">Cartão de Crédito</h4>
                      <p className="text-[10px] text-slate-400 mt-0.5">Checkout transparente sem redirecionamento</p>
                    </div>
                  </div>
                  ...
                  <div
                    onClick={() => handleUpdateCheckout(prev => ({
                      ...prev,
                      metodos: { ...prev.metodos, boleto: !prev.metodos.boleto }
                    }))}
                    className={`p-4 rounded-xl border cursor-pointer transition select-none flex flex-col justify-between gap-3 ${
                      checkoutSeguro.metodos.boleto
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        checkoutSeguro.metodos.boleto ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-500'
                      }`}>
                        {checkoutSeguro.metodos.boleto ? '✓' : ''}
                      </span>
                    </div>
                    <div>
                      <h4 className="text-xs font-black text-white">Boleto Bancário</h4>
                      <p className="text-[10px] text-slate-400 mt-0.5">Código de barras e linha digitável</p>
                    </div>
                  </div>

                </div>

                {!checkoutSeguro.metodos.pix && !checkoutSeguro.metodos.cartao && !checkoutSeguro.metodos.boleto && (
                  <p className="text-[11px] text-red-400 flex items-center gap-1.5 mt-1 font-semibold">
                    <AlertCircle className="w-3.5 h-3.5" />
                    Atenção: selecione ao menos um método de pagamento para permitir compras na campanha.
                  </p>
                )}
              </div>

              {/* Configurações de Parcelamento para Cartão */}
              {checkoutSeguro.metodos.cartao && (
                <div className="p-4 bg-slate-950/90 border border-blue-900/40 rounded-xl space-y-4">
                  <div className="flex items-center gap-2">
                    <CreditCard className="w-4 h-4 text-blue-400" />
                    <h4 className="text-xs font-bold text-white">Regras de Parcelamento (Cartão)</h4>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        value={checkoutSeguro.parcelasMax}
                        onChange={e => handleUpdateCheckout(prev => ({
                          ...prev,
                          parcelasMax: Number(e.target.value)
                        }))}
                    ...
                    <div>
                      ...
                      <select
                        value={checkoutSeguro.taxaParcelamento}
                        onChange={e => handleUpdateCheckout(prev => ({
                          ...prev,
                          taxaParcelamento: e.target.value as 'comprador' | 'organizador'
                        }))}
                        className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-blue-500"
                      >
                        <option value="comprador">Repassar juros ao Comprador (Padrão)</option>
                        <option value="organizador">Assumir juros (Sem juros para o cliente)</option>
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-white">Selos de Confiança & Segurança</h4>
                    <p className="text-[11px] text-slate-400">Exibir badges de checkout seguro (Mercado Pago, SSL 256-bit)</p>
                  </div>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={checkoutSeguro.selosSeguranca}
                    onChange={e => handleUpdateCheckout(prev => ({ ...prev, selosSeguranca: e.target.checked }))}
                    className="sr-only peer"
                  />
                  <div className="w-9 h-5 bg-slate-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-emerald-500"></div>
                </label>
              ...
              <div className="space-y-3 pt-2">
                <label className="text-xs font-bold text-slate-200 block">
                  Mensagens Customizadas no Checkout
                </label>

                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] font-semibold text-slate-400 block mb-1">
                      Mensagem de Urgência (Topo do Modal):
                    </label>
                    <input
                      type="text"
                      value={checkoutSeguro.mensagens.urgencia || ''}
                      onChange={e => handleUpdateCheckout(prev => ({
                        ...prev,
                        mensagens: { ...prev.mensagens, urgencia: e.target.value }
                      }))}
                      placeholder="Ex: Seus números estão reservados por tempo limitado!"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={checkoutSeguro.mensagens.pix || ''}
                      onChange={e => handleUpdateCheckout(prev => ({
                        ...prev,
                        mensagens: { ...prev.mensagens, pix: e.target.value }
                      }))}
                      placeholder="Ex: Aprovação imediata via Pix com QR Code e Copia e Cola."
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={checkoutSeguro.mensagens.sucesso || ''}
                      onChange={e => handleUpdateCheckout(prev => ({
                        ...prev,
                        mensagens: { ...prev.mensagens, sucesso: e.target.value }
                      }))}
                      placeholder="Ex: Seu pagamento foi confirmado! Seus números foram gerados com sucesso."
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                </div>
